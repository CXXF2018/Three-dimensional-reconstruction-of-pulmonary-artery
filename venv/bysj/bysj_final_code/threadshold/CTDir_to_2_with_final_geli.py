...
def CT_to_2(mm,dir_name,CT_name,count,geli1,geli2,geli3,thread):
    # 读取单张Dicom图像
    ...
    # 对图像进行阈值分割
    ret, img = cv2.threshold(img, thread, 3071, cv2.THRESH_BINARY)
    ...
    io.imsave('thred_'+str(thread)+'_image_with_geli/'+str(count)+'.jpg',img)
...
if __name__ == '__main__':
    ...
    geli1 = image.imread("testgeli.jpg")
    geli2 = image.imread("before155.jpg")
    geli3 = image.imread("before100.jpg")

    for ct_name in listdir:
        count=count+1
        if count >280 :
            break
        CT_to_2(mm,file_dir,ct_name,count,geli1,geli2,geli3,450)
