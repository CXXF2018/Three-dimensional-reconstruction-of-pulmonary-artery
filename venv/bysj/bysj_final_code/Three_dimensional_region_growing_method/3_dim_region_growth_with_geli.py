...
def regionGrow(images, seeds, thresh):
    ...
    while (len(seedList) > 0):
        ...
        #种子点是否符合要求
        if images[currentPoint.z][currentPoint.x, currentPoint.y] <150 :
            continue
    ...
    getNeedRegion(result,thresh)

#去除心脏
def getNeedRegion(images,thresh):
#220以后的删除
    ...
    while(len(images)>0):
        count = count + 1
        if (count > 150):
            for j in range(100,400):  # 遍历行
                for i in range(100,400):  # 遍历列
                    if img[j, i] >0:
                        images[0][j, i] = 0  # 写入像素点值
        elif(count>100):
            for j in range(100,400):  # 遍历行
                for i in range(100,400):  # 遍历列
                    if img2[j, i] >0:
        ...
        im = images.pop(0)
        io.imsave('150_thred_'+str(thresh)+'_image/'+str(count) + '.jpg', im)
...
if __name__ == '__main__':
    ...
    res = regionGrow(images, seeds, 200)
